...
/**
 * 规范化外部链接：缺省协议时自动补全 https://
 * 已带协议(http/https/mailto/tel)或站内相对路径(/ 开头)保持不变
 * 例如 www.baidu.com -> https://www.baidu.com
 * @param {string} str
 * @returns {string}
 */
export function normalizeExternalUrl(str) {
  if (!str) return ''
  str = str.trim()
  if (/^(https?:\/\/|mailto:|tel:)/i.test(str) || str.startsWith('/')) {
    return str
  }
  return 'https://' + str
}
