...
import { deepClone, normalizeExternalUrl } from '@/lib/utils'
 ...
 * 「最近在做什么」卡片数据
 *
 * 数据源 = Notion 库里 `type=Now / status=Published` 的那条页面：
 *   - 状态句 ← 页面 title（人可在 Notion 里直接改）
 *   - 在做 / 最近产出 ← summary 属性里的紧凑 JSON（由本机 ~/dsh-blog-sync/sync_now.mjs 写入）
 *
 * 只读「已经拉到的 collectionData」，**不额外打 Notion API**（零额外延迟）。
 * 页面每 60s ISR 重新验证时会重取，所以脚本写 Notion 即可生效、**不需要重新部署**。
 */
function getNowData(post) {
  if (!post) return null

  const status = (post.title || '').trim()
  let lists = null
  try {
    lists = post.summary ? JSON.parse(post.summary) : null
  } catch (e) {
    console.warn('[getNowData] summary 不是合法 JSON，本次只渲染状态句')
  }

  const doing = Array.isArray(lists?.doing) ? lists.doing.slice(0, 5) : []
  const recent = Array.isArray(lists?.recent) ? lists.recent.slice(0, 5) : []
  if (!status && doing.length === 0 && recent.length === 0) return null
  ...
  return {
    ...
    updatedAtLabel: lists?.updatedAtLabel || '',
    stats: lists?.stats || null
  }
}
...
async function convertNotionToSiteData(
  ...
  // 「最近在做什么」卡片：type=Now 的页面（不额外打 API，直接用已拉到的属性）
  const now = getNowData(collectionData.find(post => post?.type === 'Now' && post.status === 'Published'))
  ...
  // 作品集：读取 type=Portfolio 且已发布的记录
  const portfolio = collectionData
    .filter(post => post?.type === 'Portfolio' && post?.status === 'Published')
    .map(item => ({
      id: item.id,
      title: item.title || '',
      summary: item.summary || item.description || '',
      url: normalizeExternalUrl(item.url || item.link || ''),
      slug: item.slug || '',
...
function handleDataBeforeReturn(db) {
  ...
  // 作品集数据清理：过滤无效项、清理标签
  if (Array.isArray(db.portfolio)) {
    db.portfolio = db.portfolio.filter(item => item && (item.title || item.url))
    db.portfolio = cleanPages(db.portfolio, db.tagOptions)
  } else {
    db.portfolio = []
  }
